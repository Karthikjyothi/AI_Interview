...
from utils.fallback_parser import extract_name_fallback, extract_skills_fallback
...
def extract_resume_info_using_llm(resume_content):
    ...
    if response is None:
        ...
        name = extract_name_fallback(resume_content)
        highlights = extract_skills_fallback(resume_content)
        return name, highlights

    name = response.get("name", "Unknown")
    resume_highlights = response.get("highlights", [])
